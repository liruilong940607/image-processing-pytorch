...
class DeepLabV3Plus(nn.Sequential):
    ...
    def forward(self, x):
        ...
        h = self.aspp(h)
        h = self.fc1(h)
        h = F.upsample(h, size=h_.shape[2:], mode='bilinear', align_corners=True)
        h = torch.cat((h, h_), dim=1)
        h = self.fc2(h)
        h = F.upsample(h, scale_factor=4, mode='bilinear', align_corners=True)
        return h
...
if __name__ == '__main__':
    model = DeepLabV3Plus(n_classes=18, n_blocks=[3, 4, 23, 3], pyramids=[6, 12, 18]).cuda()
    model.freeze_bn()
    model.eval()
    print list(model.named_children())
    image = torch.autograd.Variable(torch.randn(1, 3, 513, 513)).cuda()
